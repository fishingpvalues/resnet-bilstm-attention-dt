...
def cauchy_combination_test(p_values: List[float]) -> float:
    """
    Implements the Cauchy Combination Test (CCT) for combining p-values
    ...
    Returns:
        Combined p-value using CCT method
    """
    # Handle empty list or list with NaN values
    p_values = [p for p in p_values if not np.isnan(p)]
    if not p_values:
        return float("nan")

    # Apply Cauchy transformation to each p-value
    # tan((0.5 - p) * pi)
    transformed = np.tan((0.5 - np.array(p_values)) * np.pi)

    # Calculate the test statistic (mean of transformed values)
    test_statistic = np.mean(transformed)

    # Convert back to p-value
    combined_p = 0.5 - np.arctan(test_statistic) / np.pi

    return combined_p


def binomial_test_rejection_rate(
    rejections: List[int], alpha: float = 0.01
) -> Tuple[float, float, float]:
    """
    Performs a binomial test on the rejection rate.

    Args:
        rejections: List of 1's (reject) and 0's (fail to reject)
        alpha: Significance level used in the original tests

    Returns:
        Tuple containing (rejection_rate, p_value, significant)
    """
    # Handle empty list or list with NaN values
    rejections = [r for r in rejections if not np.isnan(r)]
    if not rejections:
    ...
    rejection_rate = n_rejections / n_tests if n_tests > 0 else 0

    # Perform binomial test
    # Under the null hypothesis, probability of rejection is alpha
    # We test whether rejection rate is higher than expected by chance
    p_value = 1.0  # Default if test cannot be performed

    try:
        # Use scipy.stats.binomtest for the binomial test
        # We're testing if observed proportion is greater than alpha
        binom_result = stats.binomtest(
            n_rejections, n_tests, alpha, alternative="greater"
        )
        p_value = binom_result.pvalue
    except Exception as e:
        print(f"{RED}Error in binomial test: {e}{RESET}")
        # If test fails, use a conservative p-value of 1.0
        p_value = 1.0

    # Consider the result significant if p_value < 0.05
    significant = p_value < 0.05

    return rejection_rate, p_value, significant
...
def multiple_runs_hypothesis_test(
    ...
    **model_kwargs,
    ...
    for component, features in feature_subsets.items():
        print(f"\n{CYAN}Testing component: {component}{RESET}")
        ...
        for run in range(n_runs):
            print(f"{BLUE}Run {run+1}/{n_runs}{RESET}")
            ...
            try:
                ...
                # Print colored status based on significance
                if p_value < alpha:
                    status = f"{GREEN}reject H0{RESET}"
                else:
                    status = f"{RED}fail to reject H0{RESET}"

                print(
                    f"  {metric}: {observed_stat:.4f}, p-value: {p_value:.4f}, {status}"
                )

            except Exception as e:
                print(f"{RED}Error in run {run+1}: {str(e)}{RESET}")
        ...
        if valid_observations:
            ...
            # Calculate CCT p-value
            results[component]["cct_p_value"] = cauchy_combination_test(valid_p_values)

            # Calculate binomial test on rejection rate
            rr, binom_p, binom_sig = binomial_test_rejection_rate(
                valid_rejections, alpha
            )
            results[component]["binom_p_value"] = binom_p
            results[component]["binom_significant"] = binom_sig

            # Print colored summary
            print(f"\n{YELLOW}Summary for {component}:{RESET}")
            print(f"  Mean {metric}: {results[component]['mean_observed_stat']:.4f}")
            print(f"  Mean p-value: {results[component]['mean_p_value']:.4f}")
            print(f"  CCT p-value: {results[component]['cct_p_value']:.4f}")
            print(f"  Rejection rate: {results[component]['rejection_rate']:.2f}")
            print(f"  Binomial test p-value: {results[component]['binom_p_value']:.4f}")

            # Print conclusion based on CCT and binomial test
            if results[component]["cct_p_value"] < 0.05:
                print(f"  {GREEN}CCT: Significant evidence against H0{RESET}")
            else:
                print(f"  {RED}CCT: No significant evidence against H0{RESET}")

            if results[component]["binom_significant"]:
                print(
                    f"  {GREEN}Binomial: Significant rejection rate (p={results[component]['binom_p_value']:.4f}){RESET}"
                )
            else:
                print(
                    f"  {RED}Binomial: Non-significant rejection rate (p={results[component]['binom_p_value']:.4f}){RESET}"
                )
            ...
            results[component]["cct_p_value"] = float("nan")
            results[component]["binom_p_value"] = float("nan")
            results[component]["binom_significant"] = False
            print(f"{RED}No valid results for {component}{RESET}")
    ...
    with open(result_file, "w") as f:
        ...
        for component, res in results.items():
            ...
            f.write(f"  CCT p-value: {res.get('cct_p_value', float('nan')):.4f}\n")
            f.write(f"  Rejection rate: {res.get('rejection_rate', 0):.2f}\n")
            f.write(
                f"  Binomial test p-value: {res.get('binom_p_value', float('nan')):.4f}\n"
            )
            f.write(f"  Binomial significant: {res.get('binom_significant', False)}\n")
            f.write("\n")

    print(f"\n{GREEN}Results saved to: {result_file}{RESET}")

    # Generate plots for each component
    print(f"\n{CYAN}Generating plots...{RESET}")
    for component in feature_subsets.keys():
        try:
            generate_hypothesis_test_plots(model_output_dir, component)
        except Exception as e:
            print(f"{RED}Error generating plots for {component}: {e}{RESET}")

    # Generate summary plots
    try:
        generate_summary_plots(results, model_output_dir, metric, model_type)
    except Exception as e:
        print(f"{RED}Error generating summary plots: {e}{RESET}")
...
def generate_summary_plots(
    ...
    for component, res in results.items():
        ...
        cct_p_values.append(res.get("cct_p_value", float("nan")))
        rejection_rates.append(res.get("rejection_rate", 0))
        binom_p_values.append(res.get("binom_p_value", float("nan")))
        binom_significant.append(res.get("binom_significant", False))
    ...
    df = pd.DataFrame(
        {
            ...
            "CCT p-value": cct_p_values,
            "Rejection Rate": rejection_rates,
            "Binomial p-value": binom_p_values,
            "Binomial Significant": binom_significant,
        }
    )
    ...
    # 2. Plot Rejection Rates with Binomial Test Results
    fig, ax = plt.subplots(figsize=(12, 7))

    # Use different colors based on binomial significance
    bar_colors = ["darkred" if sig else "salmon" for sig in df["Binomial Significant"]]
    bars = ax.bar(df["Component"], df["Rejection Rate"], color=bar_colors, alpha=0.8)

    # Add text showing the exact values and binomial p-values
    for i, bar in enumerate(bars):
        height = bar.get_height()
        binom_p = df["Binomial p-value"].iloc[i]

        if not np.isnan(binom_p):
            # Show rejection rate and binomial p-value
            text = f"{height:.2f}\n(p={binom_p:.4f})"
        else:
            text = f"{height:.2f}"
    ...
    # Create custom legend for binomial significance
    from matplotlib.patches import Patch

    legend_elements = [
        Patch(facecolor="darkred", alpha=0.8, label="Binomial Significant (p<0.05)"),
        Patch(facecolor="salmon", alpha=0.8, label="Not Significant"),
    ]
    ...
    # 3. Plot Mean p-values and CCT p-values
    fig, ax = plt.subplots(figsize=(12, 7))
    ...
    # Add horizontal lines at 0.05 and 0.01 (common significance levels)
    ax.axhline(y=0.05, color="red", linestyle="--", alpha=0.7, label="α = 0.05")
    ax.axhline(y=0.01, color="darkred", linestyle="-.", alpha=0.7, label="α = 0.01")

    # Add text showing the exact values
    for bars, values in [(bars1, df["Mean p-value"]), (bars2, df["CCT p-value"])]:
        for i, bar in enumerate(bars):
            height = bar.get_height()
            if not np.isnan(height) and height > 0.01:  # Only show non-nan values
                ax.text(
                    bar.get_x() + bar.get_width() / 2.0,
                    height + 0.01,
                    f"{height:.4f}",
                    ha="center",
                    va="bottom",
                    rotation=90 if height > 0.5 else 0,
                    fontsize=8,
                )

    ax.set_title(f"P-values by SBDT Component ({model_type.upper()})")
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"{model_type}_p_values_comparison.png"), dpi=300
    )
    plt.close()

    # 4. Plot individual p-values (Mean p-values)
    fig, ax = plt.subplots(figsize=(12, 7))
    ...
    # 5. Plot individual CCT p-values
    fig, ax = plt.subplots(figsize=(12, 7))
    bars = ax.bar(df["Component"], df["CCT p-value"], color="mediumseagreen", alpha=0.8)
    ...
    ax.set_title(f"CCT p-value by SBDT Component ({model_type.upper()})")
    ax.set_ylabel("CCT p-value")
    ax.set_ylim(0, max(1.0, df["CCT p-value"].max() + 0.1))
    plt.xticks(rotation=45, ha="right")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"{model_type}_cct_p_values.png"), dpi=300)
    plt.close()
